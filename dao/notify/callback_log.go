...
type CallbackLog struct {
	...
	HTTPCode            int                     `gorm:"default:null"`
...
func (trd *CallbackLogDao) Save(ctx context.Context, model *CallbackLog) (uint64, error) {
...
func (trd *CallbackLogDao) Update(ctx context.Context, query *CallbackLogQuery) (int64, error) {
	db := utils.GetDB(ctx)
...
func (trd *CallbackLogDao) queryChain(query *CallbackLogQuery) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		structType := reflect.TypeOf(query.CallbackLog)
		structValue := reflect.ValueOf(query.CallbackLog)
		structPtrValue := reflect.ValueOf(&query.CallbackLog)
		for i := 0; i < structType.NumField(); i++ {
			if structValue.Field(i).IsZero() {
				continue
			}
...
func (trd *CallbackLogDao) equalScope(fieldName string, field interface{}) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		return db.Where(fmt.Sprintf("`%s` = ? ", fieldName), field)
	}
}

func (trd *CallbackLogDao) inScope(fieldName string, field interface{}) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
